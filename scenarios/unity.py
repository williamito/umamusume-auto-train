...
def unity_cup_function():
  ...
  while True:
    ...
    tries += 1
    if tries > 20:
      device_action.stop_bot("Select opponent button not found, please report this.", f"assets/notifications/{config.ERROR_NOTIFICATION}", volume = config.NOTIFICATION_VOLUME)
  ...
  if not select_opponent_btn and not s_rank_opponent:
    device_action.stop_bot("Select opponent and zenith race button not found, please report this.", f"assets/notifications/{config.ERROR_NOTIFICATION}", volume = config.NOTIFICATION_VOLUME)
  ...
  if len(rank_matches) == 0:
    device_action.stop_bot("Team rank not found, please report this.", f"assets/notifications/{config.ERROR_NOTIFICATION}", volume = config.NOTIFICATION_VOLUME)
  matchups = []
  ...
  for rank_match in rank_matches:
    ...
    while tries < 10:
      ...
      tries += 1
      if tries > 10:
        device_action.stop_bot("Affinity screen not found, please report this.", f"assets/notifications/{config.ERROR_NOTIFICATION}", volume = config.NOTIFICATION_VOLUME)
      device_action.flush_screenshot_cache()
